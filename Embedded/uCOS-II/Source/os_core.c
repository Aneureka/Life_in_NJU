...
#include <ucos_ii.h>
#endif
#include <stdio.h> // to print the message
...
// define edf task data for idle task
...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == TRUE) {
        ...
        {   
            ...
            tempPoint->rc = tempPoint->c; // reset the counter (c ticks for computation)
...
static void OS_SchedByEdf(void) 
{
    int earliestDDL = 1000000; // temporal lastest deadline
    int ddl;
    int isAllDelay = 1;

    OS_TCB* curTask = OSTCBList;
    OS_TCB* nextTask = OSTCBPrioTbl[OS_IDLE_PRIO];
