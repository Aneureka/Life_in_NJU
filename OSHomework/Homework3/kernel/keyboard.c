...
PUBLIC void keyboard_handler(int irq)
{
	...
	// 若当前缓冲区字节数 < 最大缓冲区字节数，将扫描码读入缓冲区，指针前移
	if (kb_in.count < KB_IN_BYTES) {
		*(kb_in.p_head) = scan_code;
		kb_in.p_head++;
		// 如果当前指针到达结尾，将指针放到最前面（重新写？ TODO）
...
PUBLIC void keyboard_read(TTY* p_tty)
{
	...
	u8	scan_code; // 扫描码
	char output[2]; // 将要打印的字符，有两个是因为后面一个为\0
	...
	if(kb_in.count > 0){
		...
		if ((key != PAUSEBREAK) && (key != PRINTSCREEN)) {
			...
			/* 如果是Make Code，则小于0x7F，与0x80做与运算将为0 */
			...
			// 如果已经开启了大写字母锁，则当前的caps标志置为相反值，如A变成a
			if (caps_lock) {
